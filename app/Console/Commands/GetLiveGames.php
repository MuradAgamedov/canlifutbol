...
class GetLiveGames extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:getLiveGames';
    ...
    public function handle()
    {
        try {
            ...
            $this->sendSlackNotification('Xəta baş verdi-getLiveGames: ' . $e->getMessage() . ' | Fayl: ' . $e->getFile() . ' | Sətir: ' . $e->getLine());
    ...
    public function getActiveGames()
    {
        ...
        foreach ($findedCountries[2] as $country) {
            ...
            $countries[str_replace("'", "", explode(',',  $country)[0])] = [
                'country_id' => Country::where('slug', $slug)->first()? Country::where('slug', $slug)->first()->country_id:null,
                'name' => str_replace("'", "", explode(',',  $country)[1]),
