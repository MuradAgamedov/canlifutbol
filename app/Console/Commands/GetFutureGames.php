...
class GetFutureGames extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:getFutureGames';
     ...
     * @var string
     */
    protected $description = 'Fetch games for the next 6 days starting from tomorrow';

    /**
     * Number of days to fetch games for.
     *
     * @var int
     */
    protected $daysToFetch = 6;
    ...
    {
        try {
            $this->info("Gelecek oyunları almaya başlıyor...");

            for ($i = 1; $i <= 7; $i++) {
                $date = \Carbon\Carbon::now()->addDays($i)->format('Y-m-d');
                $this->info("Oyunlar için tarih: {$date}");
                $this->getTomorowGames($date);
            }

            $this->info("Gelecek oyunlar başarıyla alındı.");
        } catch (\Exception $e) {
            die('GetFutureGames - xəta baş verdi: ' . $e->getMessage());
            $this->sendSlackNotification(
                'Xəta baş verdi-getFutureGames: ' . $e->getMessage() .
                    ' | Fayl: ' . $e->getFile() .
                    ' | Sətir: ' . $e->getLine()
            );
    ...
    public function getTomorowGames($data)
    {

        $url = "https://live.nowgoal17.com/ajax/SoccerAjax?type=6&date={$data}&order=league&timezone=UTC";
